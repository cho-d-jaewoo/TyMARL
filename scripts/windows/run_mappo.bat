...
REM ============================================================
REM  Experiments  --  MAPPO across all three races
REM ============================================================

echo ========================================================
echo  MAPPO on SMACv2 / protoss + terran + zerg
echo  n_rollout_threads=1 (set in harl\configs\algos_cfgs\mappo.yaml)
...
for %%S in (%SEEDS%) do (
    call :run mappo smacv2 protoss_5_vs_5 %%S
    call :run mappo smacv2 terran_5_vs_5  %%S
    call :run mappo smacv2 zerg_5_vs_5    %%S
)

echo.
echo ========================================================
echo  MAPPO slot finished at %TIME%
echo ========================================================
goto :eof
...
echo MAPPO DONE %EXP_NAME% finished at %TIME%
exit /b 0

:skip
echo MAPPO SKIP %EXP_NAME% already completed (done.flag exists)
exit /b 0

:run_failed
echo MAPPO WARN %EXP_NAME% failed exit %RC% continuing
exit /b 0

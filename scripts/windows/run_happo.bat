...
REM ============================================================
REM  Experiments  --  HAPPO across all three races
REM ============================================================

echo ========================================================
echo  HAPPO on SMACv2 / protoss + terran + zerg
echo  n_rollout_threads=1 (set in harl\configs\algos_cfgs\happo.yaml)
...
for %%S in (%SEEDS%) do (
    call :run happo smacv2 protoss_5_vs_5 %%S
    call :run happo smacv2 terran_5_vs_5  %%S
    call :run happo smacv2 zerg_5_vs_5    %%S
)

echo.
echo ========================================================
echo  HAPPO slot finished at %TIME%
echo ========================================================
goto :eof
...
echo HAPPO DONE %EXP_NAME% finished at %TIME%
exit /b 0

:skip
echo HAPPO SKIP %EXP_NAME% already completed (done.flag exists)
exit /b 0

:run_failed
echo HAPPO WARN %EXP_NAME% failed exit %RC% continuing
exit /b 0
